...
<div class="controls">
    <div></div>
    <button class="btn" ontouchstart="changeDir(0,-1)" onclick="changeDir(0,-1)">⬆️</button>
    <div></div>
    <button class="btn" ontouchstart="changeDir(-1,0)" onclick="changeDir(-1,0)">⬅️</button>
    <button class="btn" ontouchstart="changeDir(0,1)" onclick="changeDir(0,1)">⬇️</button>
    <button class="btn" ontouchstart="changeDir(1,0)" onclick="changeDir(1,0)">➡️</button>
</div>
...
<script>
    ...
    let snake, food, star, dx, dy, score, bestScore, gameLoop;
    let foodCount, starActive, starTimer, starInterval;
    let paused = false;
    let speed = 150;
    ...
    function placeStar() {
        ...
        starInterval = setInterval(() => {
            ...
            let pct = ((5 - elapsed) / 5) * 100;
            document.getElementById('starTimerBar').style.width = pct + '%';
            if(elapsed >= 5) {
    ...
    function getStarPoints(timeLeft) {
        if(timeLeft >= 4.5) return 50;
        if(timeLeft >= 3.5) return 40;
        if(timeLeft >= 2.5) return 30;
        if(timeLeft >= 1.5) return 20;
        return 10;
    }
    ...
    function update() {
        ...
        if(starActive && star && head.x === star.x && head.y === star.y) {
            let pts = getStarPoints(star.timeLeft) * speedMultiplier;
    ...
    function draw() {
        ...
        // Star
        if(starActive && star) {
            drawStar(star.x*box+box/2, star.y*box+box/2, 5, box/2-1, box/4);
        }
    ...
    function gameOver() {
        ...
        document.getElementById('pauseBtn').innerText = '🔄 Restart';
